...
test('[REC-03] une recette de format 1 se migre jusqu’au format 4 sans changer ses palettes', () => {
  assert.equal(FORMAT_RECETTE, 4);
  ...
  assert.equal(classement.recette.formatVersion, 4);
});

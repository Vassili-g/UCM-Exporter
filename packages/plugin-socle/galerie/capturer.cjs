/**
 * Capture la galerie d'un plugin avec le Chrome installé sur le poste.
 */
const fs = require('fs');
const os = require('os');
const path = require('path');
const { spawnSync } = require('child_process');
const { MODES, sortieDe } = require('./banc.cjs');
...
/**
 * Capture les planches, ou chaque état avec `--etats`, des modes demandés en
 * arguments (tous sans argument).
 */
function capturerGalerie(galerie, arguments_) {
  /** Marges de la planche, reprises de `banc.cjs`. */
  const planche = { largeur: 2 * galerie.largeur + 3 * 12, hauteur: 2 * (galerie.hauteur + 21) + 3 * 12 };
  ...
  try {
    for (const mode of modes) {
      ...
      const pages = parEtat
        ? galerie.etats.filter((etat) => etat.existe).map((etat) => etat.id)
        : fs
            .readdirSync(dossier)
      ...
      for (const nom of pages) {
        capturer(
          ...
          parEtat ? galerie.largeur : planche.largeur,
          parEtat ? galerie.hauteur : planche.hauteur,
        );
        comptees += 1;
      }
    }
...
module.exports = { capturerGalerie };

...
/** La version du format de tokens que le moteur écrit et que ce paquet lit. */
export const TOKENS_FORMAT_VERSION = 1;
 ...
 */
export type ValeurDeToken = CouleurDeToken | DimensionDeToken | number | string | boolean | null;
 ...
 * - `courante` : la version que ce paquet lit ;
 ...
 *
 * Un entier positif inférieur à la version courante est invalide : la seule
 * forme antérieure est la forme d'origine, qui ne porte pas de marque. Une
 * version suivante devra nommer ici la version qu'elle remplace.
 */

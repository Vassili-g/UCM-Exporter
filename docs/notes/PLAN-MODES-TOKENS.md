...
| mode | une valeur de cet axe, sous son nom normalisé : `marque1`, `marque-2` |
...
axe (`color-brand-tokens`, modes `marque1` et `marque-2`, 10 feuilles à
...
- **Preuve.** Le module CSS Custom Properties substitue `var()` au calcul de la
  ...
  sur `:root` et y vaut déjà la couleur `marque1`. Repris : dans Chrome 152,
...
:root { --ucm-mode-color-brand-tokens: marque1; }

[data-brand="marque1"] {
  --ucm-mode-color-brand-tokens: marque1;
...
- **Revenir au défaut.** Poser le nom du défaut, `data-brand="marque1"`.
...
2. Galerie : une bascule de marque sur `<html>`, une case qui place `Alert` en
   `marque-2` dans une page `marque1`, puis l'imbrication inverse. Mesure de

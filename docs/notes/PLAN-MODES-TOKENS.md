...
| mode | une valeur de cet axe, un contexte du modifier, sous son nom normalisé : `intencial`, `marque-2` |
...
`intencial` et `marque-2`, 10 feuilles à modes). Le cône de l'axe compte 60
...
```json
{
  ...
  "modifiers": {
    "color-brand-tokens": {
      "default": "intencial",
      "contexts": {
        "intencial": [{ "color-brand-tokens": { "primary": { "default": {
          "$type": "color", "$value": "{color.brands.intencial.primary.500}" } } } }],
...
:root {
  --color-brands-intencial-primary-500: color(srgb 0.07 0.31 0.62 / 1);
...
:is([data-brand="intencial"], [data-brand="marque-2"]) {
...
2. Galerie : bascule de marque sur `<html>`, `Alert` en `marque-2` dans une page
   `intencial`, puis l'inverse ; couleur calculée du fond du bouton dans chaque
   case.

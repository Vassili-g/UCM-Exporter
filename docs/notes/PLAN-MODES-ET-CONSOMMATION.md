...
| mode | une valeur de cet axe : `intencial`, `marque-2`, `dark` |
...
1. **Le contrat n'a pas besoin de changer pour les modes.** `Button` cite
   ...
   référence est la même dans les deux marques. Le nom `intencial` n'apparaît
   qu'en résolvant la chaîne d'alias. La prémisse « le bouton sait seulement que
   primary vaut intencial » décrit le CSS que le Playground génère aujourd'hui.
...
7. **Comparer les références littéralement reste juste.** Sous plusieurs modes,
   ...
   tous les modes laisserait passer un bouton figé sur `intencial`.
...
| Trois feuilles à modes dont le chemin `marque-2` ne se déduit pas du chemin `intencial` | Sa feuille de modes nomme les variables par `tokenCssVariable`, alors que le Playground en recopie une variante ASCII (section 3.4) |
...
| `color-brands` | 44 | 0 | un ; palettes `intencial` et `marque-2` en sous-groupes |
| `layouts` | 31 | 0 | un |
| `tests` | 14 | 0 | un |
| `color-brand-tokens` | 10 | 10 | `intencial`, `marque-2` |

```text
components.button.colors.primary.contained.default.background
  -> color-brand-tokens.primary.default            (modes intencial, marque-2)
       intencial : {color-brands.intencial.primary.500} -> primitives.colors.terracota.500
...
de la marque dans le chemin `intencial` peindrait trois couleurs fausses.
...
```json
{
  "color-brand-tokens": {
    "$extensions": {
      "com.ucm.axis": { "modes": ["intencial", "marque-2"], "default": "intencial" }
    },
    "primary": {
      "default": {
        "$type": "color",
        "$value": "{color-brands.intencial.primary.500}",
        "$extensions": {
          "com.ucm.modes": {
            "intencial": "{color-brands.intencial.primary.500}",
            "marque-2": "{color-brands.marque-2.primary.500}"
          }
        }
      }
...
- **Aucune convention de nom.** Un mode `Intencial/Dark` reste un mode. Le kit
...
donc pas l'identité. Sous `marque-2`, accepter la valeur `intencial` ou la valeur
...
3. Playground : générer la feuille de modes, poser une bascule de marque sur
   ...
   dans une page `intencial`, puis l'imbrication inverse.

...
> Il développe la [piste 3.3](../Evolutions%20globales/PISTES-EVOLUTION.md#33-comparer-deux-exports-et-identifier-leur-impact)
...
modification étrangère qui recouvre un fichier visé, un contrôle de référence
rouge, une autorisation absente ou un écart entre le code et une décision de ce
plan.
...
- les chemins `components` et `tokens` employés pour trouver ces fichiers.
...
Les chemins `components` et `tokens` doivent être relatifs au repository,
normalisés et rester sous sa racine. Un chemin absolu ou qui remonte au-dessus
de la racine rend l'instantané illisible. Le côté droit lit tous les contrats
...
présence d'un seul côté produit les ajouts ou retraits de feuilles et de
métadonnées définis par D7.
...
Chaque étape retire les contrats qu'elle rapproche avant de passer à la
suivante. L'unicité se juge parmi les contrats encore candidats à cette étape.
...
Le résultat sépare sept domaines. La projection d'un contrat emploie les six
premiers ; le fichier de tokens emploie le dernier.
...
qui vivent sous une prop ou un état. Un test d'exhaustivité emploie un corpus
qui couvre chaque famille de champ public. Il mute chaque feuille et refuse
toute feuille non classée ou classée deux fois.
...
La déclaration des axes, `$extensions["com.ucm.axes"]`, se compare axe par axe :
ses modes, son défaut et ses extensions avec leur parente. Sur une feuille,
`com.ucm.axis` et chaque surcharge de `com.ucm.extensions` se comparent comme
les modes. Un axe présent d'un côté et écarté de l'autre produit un fait sur la
déclaration et un fait par feuille qui perd `com.ucm.axis`.
...
Une modification du type effectif, de `$value`, d'un mode ou d'une extension
de feuille déclenche cette propagation. Une métadonnée de groupe ne la
déclenche pas. Un `$type` hérité reste traité comme une modification des
feuilles concernées.
...
sémantique du domaine, avec l'échappement défini par ce format. Un fait sur un
contrat entier emploie le pointeur vide. Un changement de token porte un
artefact de type `token`, identifié par son chemin pointé, et un pointeur dans
sa feuille.
...
Le terminal affiche chaque chemin sans le tronquer. Le résumé d'une valeur tient
sur une ligne et ne dépasse pas 120 points de code. Une valeur composite rend
son type et sa cardinalité ; le JSON reste le seul porteur de son contenu
complet. Les statuts `partial` et `failed` affichent chaque entrée de `issues`
avant le résumé des changements conservés.

La coordonnée d'un variant est l'objet normalisé de D5. Lorsque plusieurs
occurrences strictement égales produisent le même fait, `count` porte leur
cardinalité ; son absence vaut un. Aucun ordinal d'occurrence n'entre dans la
sortie.
...
le côté, le chemin et un code stable. `inputs` porte les étiquettes, les chemins
de source et l'identifiant de commit résolu pour la base ; il ne porte ni chemin
absolu ni date courante. Une variation du seul motif `implementation` n'entre
...
| mode renommé dans un axe | retrait et ajout sur la déclaration et sur chaque feuille, aucun renommage supposé |
| défaut d'un axe changé | un fait sur la déclaration de l'axe |
| axe écarté par l'export | un fait sur la déclaration, un fait par feuille qui perd `com.ucm.axis` |
| cible d'alias changée dans un seul mode | dépendance modifiée dans ce mode seulement, impacts transitifs rendus |
| surcharge d'extension ajoutée ou retirée | fait situé à l'extension et au mode, impacts transitifs rendus |
...
| chemin `components` ou `tokens` absolu ou sortant du repository | code `2`, aucun fichier extérieur lu |
...
| valeur composite ou chaîne longue | résumé terminal limité à 120 points de code |
...
Le lot L0 crée `docs/notes/Recherches/Diff Sémantique/PREUVES-DIFF-SEMANTIQUE.md`. Ce fichier sert de
...
- `docs/notes/Recherches/Diff Sémantique/PREUVES-DIFF-SEMANTIQUE.md` ;
...
- [ ] Choisir deux paires d'instantanés suivis dans l'historique : une avec un
      changement réel, une avec du bruit de catalogage si elle existe.
- [ ] Vérifier que ces instantanés couvrent un alias modifié et une dépendance
      composée modifiée ; compléter le corpus si nécessaire.
...
Critère de sortie : le corpus contient un changement normatif, un changement de
représentation, un changement d'alias et un changement de composition. Un cas
introuvable dans l'historique reçoit une fixture synthétique séparée. Aucune
réserve de corpus ne reste ouverte avant L1.
...
- [ ] Définir `diffFormatVersion`, `status`, les cinq collections et leurs
      discriminants.
- [ ] Définir les références d'artefacts avant/après, les JSON Pointers et la
      forme des coordonnées dupliquées.
- [ ] Écrire le tri déterministe du résultat.
- [ ] Séparer `changes` et `impacts` dans les types et les tests.
- [ ] Représenter les statuts `partial` et `failed` sans exception non
      contrôlée.
...
Critère de sortie : le schéma public représente chaque ligne de la matrice,
dont un renommage de contrat et les deux formes d'échec. Toute modification de
ce schéma avant H1 revient en L1 et garde `diffFormatVersion: 1` tant qu'aucune
version n'a été publiée.
...
- [ ] Renuméroter temporairement tous les catalogues d'un fixture ; le résultat
      doit rester vide.
- [ ] Muter seulement `meta.contractVersion` entre 12.0 et 13.0 sur un contrat
      compatible avec les deux grammaires ; le résultat doit rester vide.
- [ ] Croiser `componentKey` et `nodeId` pour prouver la priorité de D4.
...
- [ ] Couvrir chaque feuille publique par le test d'exhaustivité des domaines.
...
- [ ] Valider la forme des groupes, feuilles, modes et références avant leur
      comparaison.
...
- [ ] Remplacer les noms résolus par les identités rapprochées avant l'union.
...
- [ ] Résoudre la révision en identifiant de commit avant tout accès à son
      contenu.
- [ ] Refuser les chemins `components` et `tokens` absolus ou sortant de la
      racine.
...
- [ ] Borner chaque résumé de valeur à 120 points de code sans tronquer le
      chemin.
...
- [ ] Monter les versions du kit, du CLI et des paquets dont la dépendance
      exacte vers le kit change ; mettre à jour le lockfile.
- [ ] Passer deux fois `npm test`, `npm run typecheck` et `npm run build` après
      cette montée de version.
...
- [ ] Fermer le journal sans réserve ouverte. Une limite acceptée par H1 devient
      une décision de fermeture, pas une réserve.
...
- [ ] un artefact invalide produit `failed` sans changement partiel ;
- [ ] les identités ambiguës produisent `partial` et restent dans `uncompared` ;
- [ ] la priorité `componentKey`, puis `nodeId`, puis chemin reste celle du kit ;
- [ ] une renumérotation de catalogue ne produit aucun changement ;
- [ ] les seules marques de versions lues ne produisent aucun changement ;
...
- [ ] si H1 adopte la commande, les archives installées exposent l'API et la
      commande éprouvées ;
- [ ] si H1 ne garde que la bibliothèque, l'archive du CLI n'expose plus la
      commande ;
- [ ] si H1 retire l'expérimentation, aucune surface correspondante ne reste
      dans les archives ;
...
L'agent livre alors le diff final, les commandes exécutées, les résultats des
mutations, la décision issue de H1 et les commits lorsqu'ils ont été autorisés.
